...
async def save_feedback(payload: FeedbackPayload) -> JSONResponse:
    """
    Persist a reviewed order to SQLite.
    This replaces the old JSONL approach and enables proper querying
    for the closed-loop learning analysis.
    """
...
async def list_feedback(limit: int = 50) -> JSONResponse:
    ...
    Useful for closed-loop analysis and model performance tracking.
    """
    db_path = _get_db_path()
